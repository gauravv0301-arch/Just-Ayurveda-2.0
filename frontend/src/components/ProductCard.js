...
export default function ProductCard({ product, onViewDetails, index }) {
  ...
  return (
    ...
    >
      {/* Image */}
      <button
        data-testid={`product-image-btn-${product.id}`}
      ...
      <div className="p-5">
        ...
        <button
          data-testid={`product-name-btn-${product.id}`}
          onClick={() => onViewDetails(product)}
          className="text-left w-full"
        >
          <h3 className="font-['Outfit'] font-semibold text-[#233232] text-lg leading-snug hover:text-[#3bb44b] transition-colors">
            {product.name}
          </h3>
        </button>

        <p className="text-sm text-[#4f5958] mt-2 leading-relaxed line-clamp-2">
        ...
        <div className="flex flex-wrap gap-1.5 mt-3">
          {product.highlights.slice(0, 2).map((h, i) => (
            <span key={i} className="text-xs bg-[#cfecd6]/50 text-[#4f5958] px-2.5 py-1 rounded-full">
              {h}
            </span>
          ))}
        </div>
        ...
        <div className="flex items-baseline gap-2 mt-4">
          <span className="text-xl font-bold text-[#233232] font-['Outfit']">
