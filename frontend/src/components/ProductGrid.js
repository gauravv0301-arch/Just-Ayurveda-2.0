...
export default function ProductGrid({ products, loading, onViewDetails, searchQuery }) {
  ...
  return (
    <section id="products" className="py-16 md:py-24 bg-[#edfbf0]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-12 scroll-reveal">
          ...
          <p className="mt-4 text-base text-[#4f5958] max-w-xl mx-auto">
            Carefully crafted Ayurvedic formulations for men's vitality, strength, and daily wellness.
          </p>
        </div>
          ...
          <div className="flex items-center gap-3">
            <Select value={category} onValueChange={setCategory}>
              ...
              <SelectContent className="bg-white border-[#cfecd6]">
                {categories.map(cat => (
                  <SelectItem key={cat} value={cat} className="text-[#233232] text-sm capitalize">
        ...
        {searchQuery && (
          <p className="text-sm text-[#4f5958] mb-6">
            Showing results for "<span className="font-medium text-[#3bb44b]">{searchQuery}</span>"
          </p>
        )}
        ...
        {loading ? (
            ...
            {[1, 2, 3, 4, 5].map(i => (
              <div key={i} className="rounded-2xl overflow-hidden bg-white border border-[#cfecd6]">
                ...
                <div className="p-5 space-y-3">
                  ...
                  <Skeleton className="h-4 w-2/3 skeleton-shimmer rounded-full" />
                  <div className="flex gap-2 pt-2">
                    <Skeleton className="h-10 flex-1 skeleton-shimmer rounded-full" />
                    <Skeleton className="h-10 w-14 skeleton-shimmer rounded-full" />
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="text-center py-16">
            <p className="text-[#8dac96] text-lg">No products found.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
